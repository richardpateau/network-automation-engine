...
def get_netbox():
    ...
    (
        ...
        vlan_by_id,
    ) = build_index(all_interfaces, all_ip_addresses, all_vlans)
    for device_name in all_devices:
        ...
        config_data[host_ip] = {
            ...
            "port_security": {"interfaces": {}},
            "dhcp_snooping": {"enabled_vlans": [], "interfaces": {}, "option82": False},
            "dai": {
                "arp_inspection": True,
                "enabled_vlans": [],
                "interfaces": {},
            ...
            "etherchannel": {"enabled": False, "groups": {}},
            "static_routes": [],
        }
        ...
        for device_interface in device_interfaces.get(device_name.id, []):
            ...
            if device_interface.type and device_interface.type.value not in [
                "virtual",
                "lag",
                "bridge",
            ]:
                config_data[host_ip]["interfaces"].append(
                    {
                        ...
                        "should_be_up": device_interface.custom_fields.get(
                            "should_be_up", ""
                        ),
                        "stp": {
                            "portfast": device_interface.custom_fields.get(
                                "stp_portfast", False
                            ),
                            "bpdu_guard": device_interface.custom_fields.get(
                                "stp_bpdu_guard", False
                            ),
                            "root_guard": device_interface.custom_fields.get(
                                "stp_root_guard", False
                            ),
                            "loop_guard": device_interface.custom_fields.get(
                                "stp_loop_guard", False
                            ),
                            "bpdu_filter": device_interface.custom_fields.get(
            ...
            if is_switch:
                if (
                    device_interface.mode
                    and device_interface.mode.value == "access"
                ...
                if (
                    device_interface.mode
                    and device_interface.mode.value == "tagged"
                    ...
                    config_data[host_ip]["trunk_ports"].append(
                        {
                            ...
                            "mode": "trunk",
                            "allowed_vlans": ",".join(
                                str(v["vid"]) for v in device_interface.tagged_vlans
                            ),
                        }
                    )
            if is_router:
                ...
                for interface_ip in interface_ips:
                    ...
                    if "ospf" in tags and ospf_custom:
                        ...
                        if (
                            ...
                            not in ospf_process[proc_id]["interfaces"]
                        ):
                            ospf_process[proc_id]["interfaces"][
                                device_interface.name
                            ] = {
                                ...
                                "expected_neighbors": set(
                                    ospf_custom.get("ospf_neighbors", {}).get(
                                        "expected_neighbors", []
                                    )
                                ),
        ...
        if ospf_process:
            ...
            for proc_id, proc_data in ospf_process.items():
                ...
                for interface_name, interface_data in data["interfaces"].items():
                    interfaces_payload[interface_name.lower()] = {
                        ...
                        "expected_neighbors": interface_data.get(
                            "expected_neighbors", []
                        ),
                    }
        ...
        if stp_context:
            ...
            config_data[host_ip]["stp"]["vlan_priorities"] = stp_context.get(
                "vlan_priorities"
            )
        if hsrp_context:
        ...
        if nat_context:
            # list
            nat["dynamic"].extend(nat_context.get("dynamic", []))
            nat["static"].extend(nat_context.get("static", []))
            # vs dicts
        ...
        if is_router and dhcp_context:
            ...
            config_data[host_ip]["dhcp"]["pools"].extend(dhcp_context.get("pools"))
        ...
        if snmp_context:
            ...
            config_data[host_ip]["snmp"]["contact"] = snmp_context.get("contact", "")
            config_data[host_ip]["snmp"]["hosts"].extend(snmp_context.get("hosts", []))
            config_data[host_ip]["snmp"]["location"] = snmp_context.get("location", "")
            if snmp_context.get("traps"):
                config_data[host_ip]["snmp"]["traps"] = snmp_context.get("traps", {})
        if syslog_context:
            config_data[host_ip]["syslog"]["facility"] = syslog_context.get(
                "facility", "local7"
            )
            config_data[host_ip]["syslog"]["hosts"].extend(
                syslog_context.get("hosts", [])
            )
            config_data[host_ip]["syslog"]["source_interface"] = syslog_context.get(
                "source_interface", ""
            )
            config_data[host_ip]["syslog"]["timestamps"] = syslog_context.get(
                "timestamps", False
            )
            config_data[host_ip]["syslog"]["trap_level"] = syslog_context.get(
                "trap_level", ""
            )
        ...
        if snooping_context:
            ...
            config_data[host_ip]["option82"] = snooping_context.get("option82")
        ...
        if dai_context:
            config_data[host_ip]["dai"]["arp_inspection"] = dai_context.get(
                "arp_inspection", ""
            )
            ...
            config_data[host_ip]["dai"]["log_buffer"] = dai_context.get(
                "log_buffer", {}
            )
        if cdp_context:
            config_data[host_ip]["cdp"]["enabled"] = cdp_context.get("enabled", False)
            config_data[host_ip]["cdp"]["timer"] = cdp_context.get("timer", None)
            config_data[host_ip]["cdp"]["holdtime"] = cdp_context.get("holdtime", None)
        ...
        if ether_context:
            config_data[host_ip]["etherchannel"]["enabled"] = ether_context.get(
                "enabled", False
            )
            config_data[host_ip]["etherchannel"]["groups"] = ether_context.get(
                "groups", {}
            )
        if static_context:
