...
def test_build_nat_pat(pat_netconf): 
	result, interfaces = build_nat(pat_netconf)

	assert result[0]["acl"] == 1
	assert result[0]["interface"] == "gigabitethernet1"
	assert result[0]["overload"] is True 

def test_build_nat_static(pat_netconf):
	result, interfaces = build_nat(pat_netconf)

	static = {(s["inside_local"], s["inside_global"]) for s in result}
...
def test_build_nat_dynamic(dynamic_nat_netconf): 
	result, interfaces = build_nat(dynamic_nat_netconf)
...
def test_build_nat_interfaces(pat_netconf): 
	...
	assert interfaces["inside"] == ["gigabitethernet3"]
	assert interfaces["outside"] == ["gigabitethernet1", "gigabitethernet2"]

def test_build_nat_empty():
	assert build_nat({}) == ([], {"inside": [], "outside": []})
	assert build_nat(None) == ([], {"inside": [], "outside": []})
	assert build_nat({"not_netconf": {}}) == ([], {"inside": [], "outside": []})
	assert build_nat({"native_netconf": {}}) == ([], {"inside": [], "outside": []})
...
def test_build_nat_no_nat_interface(): 
	...
	result, interfaces = build_nat(data)
	assert interfaces["inside"] == []
...
def test_build_nat_dynamic_defaults():
 ...
 def test_build_nat_static_default():
	...
	result, interfaces = build_nat(data)
	static = {(s["inside_local"], s["inside_global"]) for s in result if "inside_local" in s}
