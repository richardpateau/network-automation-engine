...
def access_trunk_genie_output():
    fixture_path = Path(__file__).parent / "fixtures" / "cli" / "access_trunk.json"
    with open(fixture_path) as f:
        return json.load(f)
...
def interface_genie_output():
    fixture_path = (
            Path(__file__).parent / "fixtures" / "cli" / "interface_genie_output.json"
    )
    with open(fixture_path) as f:
        return json.load(f)
...
def stp_global_genie_output():
    fixture_path = (
            Path(__file__).parent / "fixtures" / "cli" / "stp_genie_output.json"
    )
    with open(fixture_path) as f:
        return json.load(f)
...
def cdp_running_config():
    ...
    return {"cdp": global_path.read_text(), "cdp_interface": interface_path.read_text()}
...
def dai_running_config():
    ...
    dai_interface = (
            Path(__file__).parent / "fixtures" / "cli" / "running_config_dai_interfaces.txt"
    )

    return {
        "running_config": dai_global.read_text(),
        "dai_interfaces": dai_interface.read_text(),
    }
